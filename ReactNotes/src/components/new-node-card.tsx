...
export function NewNodeCard({ onNoteCreated }: NewNodeCardProps) {
  ...
  function handleStartRecording() {
    ...
    const isSpeechRecognitionEventAvailable = 'SpeechRecognition' in window
      || 'webkitSpeechRecognition' in window;

    if (!isSpeechRecognitionEventAvailable) {
      alert("Infelizmente, parece que seu navegador não suporta a API de gravação.");
      return;
    }
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 text-left p-5 space-y-3 overflow-hidden outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
            ...
            <form className="flex-1 flex flex-col">
              <div className="flex flex-1 flex-col gap-3 p-5">
                ...
                {shouldShowOnboarding ? (
                  <p className="text-sm leading-6 text-slate-400">
                    Inicie{" "}
                    <button type="button" className="text-lime-400 hover:underline" onClick={handleStartRecording}>
                      gravando uma nota
                    </button>{" "}
                    em áudio ou, se preferir,{" "}
                    <button type="button" onClick={handleStartEdition} className="text-lime-400 hover:underline">
                      digite apenas texto
                    </button>
                  </p>
                ) : (
